...
pub enum ExportState {
    ...
    Running { track_idx: usize, total: usize, frame_frac: f32 },
    Done,
    Error(String),
}
...
pub fn export_all_async(
    ...
    let progress: Progress    = Arc::new(Mutex::new(ExportState::Running {
        track_idx: 0, total: tracks.len(), frame_frac: 0.0,
    }));
    ...
    std::thread::spawn(move || {
        ...
        for (idx, track) in tracks.iter().enumerate() {
            ...
            *prog.lock().unwrap() = ExportState::Running {
                track_idx: idx, total, frame_frac: 0.0,
            };

            // Slice CDG data for this track only.
            let cdg_start = track.cdg_offset() as usize;
            let cdg_end   = (cdg_start + track.sectors as usize * CDG_BYTES_PER_SECTOR)
                                .min(cdg_raw.len());
            ...
            let out_path = output_dir.join(
                format!("{disc_title} - Track {:02}{suffix}.mkv", track.number)
            );

            if let Err(e) = run_export(
                &packets, &audio, cdeg_on, &out_path, &prog, idx, total, &cancel_thread,
            ) {
                *prog.lock().unwrap() = ExportState::Error(
                    format!("Track {:02}: {e}", track.number)
                );
                return;
            }
...
fn run_export(
    ...
    let mut child = Command::new("ffmpeg")
        .args([
            ...
            "-map", "1:a",          // explicitly map audio stream
            "-c:v", "libsvtav1",
            "-crf", "30",
            "-b:v", "0",
            "-preset", "6",
    ...
    while pkt_idx < packets.len() {
        ...
        for i in pkt_idx..end {
            if let (_, Some(ref pkt)) = packets[i] { screen.apply(pkt); }
        }
        pkt_idx = end;
    ...
    if status.success() {
        ...
        Err("ffmpeg exited with an error. Is libsvtav1 compiled in? Try: brew install ffmpeg".to_string())
    }
}
