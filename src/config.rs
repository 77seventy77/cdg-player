...
pub fn scan_library(library: &Path) -> Vec<DiscEntry> {
    ...
    for entry in entries.flatten() {
        ...
        if path.is_dir() {
            ...
            let cue = children
                .iter()
                .find(|p| p.extension().and_then(|e| e.to_str()) == Some("cue"));
            if let Some(cue_path) = cue {
                let title = path
                    ...
                    .to_string_lossy()
                    .into_owned();
                discs.push(DiscEntry {
                    title,
                    source: DiscSource::Cue(cue_path.clone()),
                });
            ...
            for child in &children {
                    ...
                    .as_deref()
                {
                    Some("zip") if zip_contains_cue(child) => {
                        ...
                        discs.push(DiscEntry {
                            title,
                            source: DiscSource::Zip(child.clone()),
                        });
                        break;
                    }
                    Some("7z") if sevenz_contains_cue(child) => {
                        ...
                        discs.push(DiscEntry {
                            title,
                            source: DiscSource::SevenZ(child.clone()),
                        });
                        break;
                    }
                    _ => {}
                ...
                .as_deref()
            {
                ...
                Some("zip") if zip_contains_cue(&path) => {
                    ...
                    discs.push(DiscEntry {
                        title,
                        source: DiscSource::Zip(path.clone()),
                    });
                }
                // ── Top-level 7z ──────────────────────────────────────────
                Some("7z") if sevenz_contains_cue(&path) => {
                    ...
                    discs.push(DiscEntry {
                        title,
                        source: DiscSource::SevenZ(path.clone()),
                    });
                }
                _ => {}
            }
...
fn sevenz_contains_cue(path: &Path) -> bool {
    ...
    reader
        ...
        .files
        .iter()
        .any(|e| e.name().to_ascii_lowercase().ends_with(".cue"))
}
